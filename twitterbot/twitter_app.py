...
username = st.sidebar.text_area(
    "Enter Twitter username without the '@'", twitter_username, height=50
)
...
tweet_info = [
    {
        "default_profile": jsonify_tweepy(user_data)["default_profile"],
        ...
        "profile_background_image_url": jsonify_tweepy(user_data)[
...
# create new columns
df["account_age_days"] = (
    df["probe_timestamp"].dt.date - df["created_at"].dt.date
).dt.days
